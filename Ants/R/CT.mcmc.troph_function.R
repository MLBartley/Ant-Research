#' Continuous Time MCMC Estimation function for Trophallaxis data
#'
...
CT.mcmc.troph = function(a, b, c, d, r, q, tau, theta, data, init, 
    Time, n.mcmc, save = F) {
    
    # homes
    
    params = matrix(NA, nrow = 4, ncol = n.mcmc)
    rownames(params) <- c("gamma_high^tilde", "gamma_low", "lambda_high", 
        "lambda_low")
    colnames(params) <- 1:n.mcmc
    
    X.param = matrix(NA, nrow = Time, ncol = n.mcmc, byrow = T)
    
    # probability matrix for 2 state discrete time Markov Chain
    
    # stores all M matrices for every iteration in mcmc
    M.param = matrix(data = NA, nrow = 4, ncol = n.mcmc, byrow = T)
    ...
    params[1:2, 1] = c(init[1], init[2])
    params[3:4, 1] = c(init[3], init[4])
    ...
    log.fullcond = function(params, P_L, P_H, data, X.param) {
        ...
        for (t in 2:length(data)) {
            if (X.param[t, l - 1] == 1) {
                sumP_H = sumP_H + log(P_H[data[t - 1] + 1, data[t] + 
                  1])
            } else {
                sumP_L = sumP_L + log(P_L[data[t - 1] + 1, data[t] + 
                  1])
            }
        }
        
        
        loglike = sumP_L + sumP_H + dgamma(params[1], a, b, log = T) + 
            dgamma(params[2], c, d, log = T) + dgamma(params[3], r, 
            q, log = T) + dgamma(params[4], r, q, log = T)
    ...
    for (l in 2:n.mcmc) {
        ...
        R_H.star = matrix(0, nrow = max(data) + 1, ncol = max(data) + 
            1)
        ...
        for (i in 1:nrow(R_H.star)) {
            ...
            if (i%%2 != 0 & i != nrow(R_H.star) & i != (nrow(R_H.star) - 
                1)) {
        ...
        R_L.star = matrix(0, nrow = max(data) + 1, ncol = max(data) + 
            1)
        ...
        for (i in 1:nrow(R_L.star)) {
            ...
            if (i%%2 != 0 & i != nrow(R_L.star) & i != (nrow(R_L.star) - 
                1)) {
        ...
        MHprob = exp(log.fullcond(theta.star, P_L.star, P_H.star, 
            data, X.param) - log.fullcond(params[, l - 1], P_L, P_H, 
            data, X.param))
        ...
        prob_H = P_H[data[1] + 1, data[2] + 1] * 0.5 * M[1, X.param[2, 
            l - 1]]
        
        
        prob_L = P_L[data[1] + 1, data[2] + 1] * 0.5 * M[2, X.param[2, 
            l - 1]]
        
        X.param[1, l] = sample(x = (1:2), size = 1, prob = c(prob_H, 
            prob_L))
        ...
        for (t in 2:(Time - 1)) {
            ...
            prob_H = P_H[data[t] + 1, data[t + 1] + 1] * M[1, X.param[t + 
                1, l - 1]] * M[X.param[t - 1, l - 1], 1]
            
            prob_L = P_L[data[t] + 1, data[t + 1] + 1] * M[2, X.param[t + 
                1, l - 1]] * M[X.param[t - 1, l - 1], 2]
            
            X.param[t, l] = sample(x = (1:2), 1, prob = c(prob_H, 
                prob_L))
        ...
        prob_H = P_H[data[Time - 1] + 1, data[Time] + 1] * M[X.param[1, 
            l - 1], X.param[Time - 1, l - 1]]
        
        prob_L = P_L[data[X.param[Time - 1]] + 1, data[X.param[Time]] + 
            1] * M[X.param[2, l - 1], X.param[Time - 1, l - 1]]
    ...
    # compile estimates - stop code midway version below X.est =
    # matrix(data = rep(NA, Time), nrow = Time, ncol = 1)
    # gamma.high.tilde.est = mean(params[1, ]) gamma.low.est =
    # mean(params[2, ]) lambda.high.est = mean(params[3, ])
    # lambda.low.est = mean(params[4, ]) gamma.high.est =
    # mean(params[1,] + params[2]) estimate = c(gamma.high.tilde.est,
    # gamma.low.est, lambda.high.est, lambda.low.est, gamma.high.est)
    
    
    
    para.est = apply(params, 1, bm)
    
    param.var = apply(params, 1, quantile, probs = c(0.025, 0.975), 
        na.rm = TRUE)
    ...
    # Parameters
    plot(0, 0, xlab = "MCMC Runs", ylab = "Rates (per minute)", ylim = c(0, 
        (max(params[1, ] + params[2, ]) * 60)), xlim = c(0, n.mcmc), 
        type = "n", cex.lab = 1)
    ...
    # M
    plot(0, 0, xlab = "MCMC Runs", ylab = "M", ylim = c(0, max(M.param)), 
    ...
    list(X.est = X.est, param.est = param.est, param.var = param.var, 
        M.est = M.est, M.var = M.var, accept.ratio = accept.ratio)
