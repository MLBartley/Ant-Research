...
#'
#' This function differes from DT.pen.mcmc.troph only in that it features 
#' an Exponential prior on the gamma values instead of Half Normal priors. 
#'
...
#'
#' @param   y.data, ant.file, title, ?, theta, states, n.mcmc, delta.t
...
DT.pen.mcmc.troph.expprior = function(penalty, y.data, states, ant.file, 
    hours, a, b, c, d, tau, tau.pen, n.mcmc, seconds, fig.save, start) {
    ...
    ## Build Homes for X(1:T), lambda(1:n) vector , and P(nXn), gamma
    ## (nxn) matrices
    ...
    # note that we're just collecting the off diagonal (non zero)
    # values needed to calculate P
    gamma.param = matrix(NA, nrow = n, ncol = n.mcmc, byrow = T)
    ...
    P.matrix[1, 2] = gamma.param[1, 1] * exp(-gamma.param[1, 1] * 
        seconds)/(gamma.param[1, 1]/(-1 + exp(gamma.param[1, 1])))
    P.matrix[1, 1] = 1 - P.matrix[1, 2]
    P.matrix[2, 1] = gamma.param[2, 1] * exp(-gamma.param[2, 1] * 
        seconds)/(gamma.param[1, 1]/(-1 + exp(gamma.param[1, 1])))
    ...
    log.fullcond = function(P.param, params, X.param, penalty) {
        ...
        for (t in 2:length(data)) {
            ...
            sumX = sumX + log(P.matrix[X.param[t - 1, l - 1], X.param[t, 
                l - 1]])
        }
        
        loglike = sumX - penalty * ((params[1] + params[2]))  #changed to account for new exp prior
    ...
    for (l in 2:n.mcmc) {
        ...
        # adaptive tuning parameter if(l < n.mcmc/2 & l %% 100 == 0){
        # sigma = (2.38^2 / 2) * var(log(t(gamma.param[, 1:(l - 1)]))) tau
        # = sigma }
        
        
        proposal = rmvnorm(n = 1, mean = log(gamma.param[, l - 1]), 
            sigma = tau)
        # proposal.pen = rmvnorm(n = 1, mean = log(penalty.param[, l -
        # 1]), sigma = tau.pen)
        ...
        # uncomment to hold gamma proposal fixed at true values theta.star
        # = c(.005, .005)
        ...
        P.matrix[1, 2] = theta.star[1] * exp(-theta.star[1] * seconds)/(theta.star[1]/(-1 + 
            exp(theta.star[1])))
        
        P.matrix[1, 1] = 1 - P.matrix[1, 2]
        P.matrix[2, 1] = theta.star[2] * exp(-theta.star[2] * seconds)/(theta.star[2]/(-1 + 
            exp(theta.star[2])))
        P.matrix[2, 2] = 1 - P.matrix[2, 1]
        
        P.star = as.vector(t(P.matrix))
        
        # Uncommont to hold P matrix constant P.star = P.param[,1]
        
        # calculate probability
        MHprob = exp(log.fullcond(P.star, theta.star, X.param, penalty) - 
            log.fullcond(P.param[, l - 1], gamma.param[, l - 1], X.param, 
                penalty))
        ...
        # #calculate probability MHprob.pen = exp(log.fullcond(P.param[,
        # l-1], gamma.param[, l-1], X.param, theta.star.pen) -
        # log.fullcond(P.param[, l - 1], gamma.param[, l-1], X.param,
        ...
        P.matrix = matrix(data = c(P.param[, l]), nrow = n, ncol = n, 
            byrow = T)
        ...
        for (t in 2:(Time - 1)) {
            ...
            Y.1SA.param[t, l] = lambda.low * P.matrix[X.param[t, l], 
                1] + lambda.high * P.matrix[X.param[t, l], 2]
        ...
        Y.1SA.param[Time, l] = lambda.low * P.matrix[X.param[Time, 
            l], 1] + lambda.high * P.matrix[X.param[Time, l], 2]
        
        ## Lambda Parameters
        
        lambda.param[1, l] = rgamma(n = 1, shape = sum(data[which(X.param[, 
            l] == 1)]) + a, rate = sum(m[1, ]) + b)
        
        lambda.param[2, l] = rgamma(n = 1, shape = sum(data[which(X.param[, 
            l] == 2)]) + c, rate = sum(m[2, ]) + d)
        
        # Uncomment to hold lambda values fixed at truth lambda.param[, l]
        # = lambda.start
    }
    ...
    lambda.var = apply(lambda.param, 1, quantile, probs = c(0.025, 
        0.975), na.rm = TRUE)
    
    gamma.est = apply(gamma.param, 1, bm)
    gamma.var = apply(gamma.param, 1, quantile, probs = c(0.025, 0.975), 
        na.rm = TRUE)
    ...
    # if(fig.save == T){ pdf(file = paste('./output/', Sys.time(),
    # '.pdf', sep = '')) }
    ...
    plot(0, 0, xlab = "MCMC Runs", ylab = "Rates (per second)", ylim = c(0, 
        max(gamma.param, lambda.param)/seconds), xlim = c(0, n.mcmc), 
        type = "n", cex.lab = 1)
    lines(1:n.mcmc, (lambda.param[1, ] + lambda.param[2, ])/seconds, 
        col = col[1])
    ...
    plot(0, 0, xlab = "MCMC Runs", ylab = "Probability Matrix for State Switching", 
        ylim = c(0, max(P.param)), xlim = c(0, n.mcmc), type = "n", 
        cex.lab = 1)
    for (i in 1:(4)) {
