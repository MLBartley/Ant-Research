...
#' gamma values.
#' 
#' @param   y.data, ant.file, title, a, b, c, d, theta, states,
#'   n.mcmc, delta.t
...
DT.pen.mcmc.troph = function(penalty, y.data, states, ant.file, hours, 
    a, b, c, d, tau, tau.pen, n.mcmc, seconds, fig.save, start) {
    ...
    P.param = matrix(data = rep(NA, n * n * n.mcmc), nrow = n * n, ncol = n.mcmc, 
        byrow = T)
    ...
    P.matrix[1, 2] = gamma.param[1, 1] * exp(-gamma.param[1, 1] * seconds)/(gamma.param[1, 
        1]/(-1 + exp(gamma.param[1, 1])))
    P.matrix[1, 1] = 1 - P.matrix[1, 2]
    P.matrix[2, 1] = gamma.param[2, 1] * exp(-gamma.param[2, 1] * seconds)/(gamma.param[1, 
        1]/(-1 + exp(gamma.param[1, 1])))
    ...
    for (l in 2:n.mcmc) {
        ...
        # adaptive tuning parameter if(l < n.mcmc/2 & l %% 100 == 0){ sigma =
        # (2.38^2 / 2) * var(log(t(gamma.param[, 1:(l - 1)]))) tau = sigma }
        ...
        for (t in 2:(Time - 1)) {
            ...
            gam[t, 2] = lambda.high^data[t] * exp(-lambda.high) * P.matrix[X.param[t - 
                1, l - 1], 2] * P.matrix[2, X.param[t + 1, l - 1]]
        ...
        gam[Time, 1] = lambda.low^data[Time] * exp(-lambda.low) * P.matrix[X.param[Time - 
            1, l - 1], 1]
        
        gam[Time, 2] = lambda.high^data[Time] * exp(-lambda.high) * P.matrix[X.param[Time - 
            1, l - 1], 2]
        ...
        Y.1SA.param[Time, l] = lambda.low * P.matrix[X.param[Time, l], 
            1] + lambda.high * P.matrix[X.param[Time, l], 2]
    ...
    lambda.var = apply(lambda.param, 1, quantile, probs = c(0.025, 0.975), 
        na.rm = TRUE)
    ...
    plot(0, 0, xlab = "MCMC Runs", ylab = "Probability Matrix for State Switching", 
        ylim = c(0, max(P.param)), xlim = c(0, n.mcmc), type = "n", cex.lab = 1)
    for (i in 1:(4)) {
