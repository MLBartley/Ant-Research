...
mcmc.troph = function(y.data, ant.file, title, a, b, c, d, theta, 
    states = 2, n.mcmc, delta.t, hours) {
    data = y.data
    Time = length(data)
    n = states
    delta = rep(1/n, n)
    ...
    P.matrix = matrix(data = theta/(sum(theta[1, ])), nrow = n, ncol = n, 
        byrow = T)
    ...
    for (l in 2:n.mcmc) {
        ...
        for (t in 2:(Time - 1)) {
            ...
            # for(k in 1:n){ gam[t, k] = (lambda.param[k, l - 1] ^ data[t]) *
            # exp(-lambda.param[k, l - 1]) * P.matrix[X.param[t - 1, l - 1],
            # k] * P.matrix[k, X.param[t + 1, l - 1]] }
            
            gam[t, 1] = lambda.low^data[t] * exp(-lambda.low) * P.matrix[X.param[t - 
                1, l - 1], 1] * P.matrix[k, X.param[t + 1, l - 1]]
            
            gam[t, 2] = lambda.high^data[t] * exp(-lambda.high) * 
                P.matrix[X.param[t - 1, l - 1], 2] * P.matrix[k, X.param[t + 
                1, l - 1]]
        ...
        # for(k in 1:n){ gam[Time, k] = lambda.param[k, l - 1] ^
        # data[Time] * exp(-lambda.param[k, l - 1]) *
        # P.matrix[X.param[(Time - 1), l - 1], k] }
        
        gam[Time, 1] = lambda.low^data[Time] * exp(-lambda.low) * 
            P.matrix[X.param[Time - 1, l - 1], 1]
        
        gam[t, 2] = lambda.high^data[Time] * exp(-lambda.high) * P.matrix[X.param[Time - 
            1, l - 1], 2]
        ...
        for (h in 1:n) {
            ...
            # sum(data[which(X.param[, l] == h)]) + a, rate = sum(m[h, ]) + b
            # )
    ...
    # for(i in 1:n ){ lambda.est[i, 1] = mean(lambda.scale[i, ]) }
    ...
    par(mfrow = c(2, 2), oma = c(0, 0, 2, 0) + 1, mar = c(1, 1, 1, 
        1) + 3)
    plot(0, 0, xlab = "MCMC Runs", ylab = "Lambda (scaled per 60 seconds)", 
