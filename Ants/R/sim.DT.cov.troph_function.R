# ` Simulation of Poisson Proccess HMM - with forager times
# covariate
#'
...
sim.DT.cov.troph <- function(tmax, delta.t, start.state = 1, alpha.beta, 
    lambda, num.locations = 1) {
    ...
    for (i in 2:tmax) {
        ...
        if (round(arrive.time[arrival + 1]) == i) {
            # first 'time' is just zero placeholder
            covariate[i] = 0
            arrival = arrival + 1
        } else {
    ...
    ## IN FUTURE NEED TO GENERALIZE ABOVE CODE FOR ANY NUMBER OF
    ## COVARIATES!  Current Meridith apologizes to Future Meridith, but
    ## this is on Her.
    ...
    # Want to first simulate second by second data then optionally bin
    # into delta.t segments
    
    P.12.star = gamma.star.LH.t * exp(-gamma.star.LH.t * 1)/(gamma.star.LH.t/(-1 + 
        exp(gamma.star.LH.t)))
    
    
    P.11.star = 1 - P.12.star
    
    P.21.star = gamma.star.HL.t * exp(-gamma.star.HL.t * 1)/(gamma.star.HL.t/(-1 + 
        exp(gamma.star.HL.t)))
    ...
    for (t in 2:tmax) {
        ...
        ## sample observed events y[t]=rpois(1,lambda=lambda[x[t]]*delta.t)
        y[t] = rpois(1, lambda = lambda[x[t]] * 1)
        # want to simulate data every second, but then bin by delta.t
        # aftwards
    }
    ...
    if (num.locations == 1) {
        ...
        Location = sample(1:2, size = length(start.time), replace = T, 
            prob = c(0.5, 0.5))
    }
    ...
    plot(1:(length(cumsum(y))), cumsum(y), type = "p", pch = ".", 
        cex = 2, col = x, xlab = "Time", ylab = "Interactions", main = "Full Timeline, Cumulative Interactions")
    points(arrive.time, rep(0, length(arrive.time)), pch = 8, col = "#53bc84")
    
    plot(1:tmax, y, type = "l", cex = 2, col = x, xlab = "Time", ylab = "Number of Interactions", 
        main = "Full Timeline")
    
    
    
    if (delta.t > 1) {
        plot(1:(length(cumsum(bin.y))), cumsum(bin.y), type = "p", 
            pch = ".", cex = 2, col = bin.x, xlab = "Time", ylab = "Interactions", 
            main = "Binned Intervals")
        
        list(inter.persec = y, state = x, cumu.inter = cumsum(y), 
            delta.t = delta.t, bin.inter = bin.y, bin.state = bin.x, 
            bin.sec = (0:(T - 1)) * delta.t, start_time = start.time, 
            Location = Location, covariate = covariate)
        
    } else {
        list(inter.persec = y, state = x, cumu.inter = cumsum(y), 
            delta.t = delta.t, start_time = start.time, Location = Location, 
            covariate = covariate)
    }
