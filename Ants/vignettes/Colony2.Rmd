...
##THIS ONE FOR JSM
penalize_col2lo4qbin1 <- foreach (i = exp(seq(-65, 30, by =  5)) ,
                                  .errorhandling="remove") %dopar% 
...
penalize_col2lo4qbin1 <- foreach(i = range,
  ...
  DT_pen_mcmc(penalty = i, starts_data = col2_low4_5$queen_starts_persec, 
              ...
              a = .005, b = .001, c = .005, d = .001,
              tau = tau, tau.pen = 0, n_mcmc = n_mcmc, 
              delta_t = delta_t, start = start, data_out = paste("../data/", "pen_MCMC", round(i, 11),
    ".csv",
    sep = ""))
...
```{r penalized diagnoses}

pen_results <- list()

for (r in 1:length(range)) {
  
temp_results <-  read.csv(paste("../data/", "pen_MCMC", round(range[r], 11),
    ".csv",
    sep = "")))

pen_results[[r]] <- penalty_diagnosis(temp_results, Time = hours * 60 * 60, fig_path = path,
