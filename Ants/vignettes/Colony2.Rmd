...
osa_param <- temp_results[, (11 + hours*60*60):(ncol(temp_results))]
data <- col2_low4_5$queen_starts_persec

  # sum.it <- 0
  # 
  # for (i in 2:(nrow(osa_param)/2)) { #i = 1 is starting values, meh ##CHANGE BACK TO ALL ROWS
  #   for (t in 1:(hours * 60 * 60)) {
  #     sum.it <- sum.it + (osa_param[i, t] - data[t])^2
  #   }
  # }
...
print(compare_plot)
print(compare_plot + ylim(.0177, .018)) #limits chosen via observation of first plot
```
