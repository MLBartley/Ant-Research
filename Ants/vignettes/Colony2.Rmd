...
MSPE_results <- matrix(NA, ncol = 3, nrow = length(range))
  colnames(MSPE_results) = c("log(penalty)", "MSPE", "accept")
...
osa_param <- temp_results[, (9 + hours*60*60):(ncol(temp_results))]
data <- col2_low4_5$queen_starts_persec

  sum.it <- 0

  for (i in 1:n_mcmc) {
    for (t in 1:(hours * 60 * 60)) {
...
compare_plot <- ggplot(data = MSPE, aes(log(penalty), MSPE)) +
      geom_point(aes( x = log(penalty), colour = accept <= 1000))

minMSPE <- MSPE_results[min(MSPE_results$MSPE), ]
```
...
range <- minMSPE[1] #if only want one penalty that has been optimized
