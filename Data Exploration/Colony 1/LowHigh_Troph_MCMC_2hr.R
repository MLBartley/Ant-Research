...
##
## 27 MAR 2016
## Received movement data for foraging chamber. Possible to
## incorperate as covariate. 
##
#####################
setwd("~/Google Drive/PSU/Projects/Ant-Research/Data/")
...
high <- read.csv("./Colony_1_trophallaxis_high_density.csv")
low <- read.csv("./Colony_1_trophallaxis_low_density.csv")
...
forage.low <- read.csv("./Colony1_low_foraging_2hr.csv")
forage.high <- read.csv("./Colony1_high_foraging_2hr.csv")
...
hist(table(low$Ant_ID), xlab = "Count",
     ...
     breaks = 15)
abline(v=mean(table(low$Ant_ID)), lty = 3, col="red", lwd = 3)
...
## Plots of Interaction Count over Time (2hr)
  
par(mfrow = c(1, 1))
plot(high$start_time,1:nrow(high), 
     main="High Density Trophallaxis",
     xlab = "Start Time", 
     ylab = "Number of Interactions")
points(forage.high$end, rep(0, nrow(forage.high)), 
       pch = 8, col = "red")
  
plot(low$start_time, 1:nrow(low), 
     main="Low Density Trophallaxis",
     xlab = "Start Time", 
     ylab = "Number of Interactions", 
     col=low$Location)
points(forage.low$end, rep(0, nrow(forage.low)), 
       pch=8, col="red")
#     legend(5000, 100, c("Loc1", "Loc4"), lty = c(1,1), col = c("black", "blue"))

par(mfrow = c(1, 2), oma = c(0, 0, 2, 0))
    plot(low.1$start_time, 1:nrow(low.1), 
         main="Location 1",
         xlim = c(0, 7200),
         xlab = "Start Time", 
         ylab = "Number of Interactions")
points(forage.low$end, rep(0, nrow(forage.low)), pch=8, col="red")
  
    plot(low.4$start_time, 1:nrow(low.4), 
         main="Location 4",
         xlim = c(0, 7200),
         xlab = "Start Time", 
         ylab = "Number of Interactions",
...
points(forage.high$end, rep(0, nrow(forage.high)), pch=8, col="red")


#Low Density - 2 Hours

plot(low$start_time, 1:nrow(low), main="Low", xlab="Seconds", xlim=c(0,max(high$end_time)))
##    plot(one.day,1:length(one.day),main=day,xlab="Minutes")
...
plot(low.1$start_time, 1:nrow(low.1), main="Low, Loc 1", xlab="Seconds", xlim=c(0,max(high$end_time)))
##    plot(one.day,1:length(one.day),main=day,xlab="Minutes")
...
plot(low.4$start_time, 1:nrow(low.4), main="Low, Loc 4", xlab="Seconds", xlim=c(0,max(high$end_time)))
##    plot(one.day,1:length(one.day),main=day,xlab="Minutes")
states = out.low$X.est
rr=rle(states[,1])
rr$values = round(rr$values, digits = 0)
